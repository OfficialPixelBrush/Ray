...
class Texture {
	public:
		...
		Color getRangedTexturePixel(float xRange, float yRange, Line* line) {
			...
			float xModifiedPos = (float)width  * (xRange * line->textureScale);
			float yModifiedPos = (float)height * (yRange * line->textureScale);
...
void preCalculateStepArray() {
	...
	for (int initStepCount = 0; initStepCount < WINDOW_HEIGHT_HALF; initStepCount++) {		
		StepSizeDistanceArray[initStepCount] = 
...
// PreCalculate the Level Lighting
